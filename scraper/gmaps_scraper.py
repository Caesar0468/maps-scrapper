...
def dismiss_overlays(page: Page) -> None:
    try:
        ...
        if close_drawer.is_visible(timeout=800):
            close_drawer.click()
    except Exception:
        pass
...
def scroll_feed(page: Page, max_scrolls: int = 30) -> None:
    """Scrolls continuously until the bottom of the feed is reached."""
    ...
    for _ in range(max_scrolls):
        try:
            # Stop if Google Maps displays the end-of-list message
...
def parse_list_cards(page: Page, locality: str, fallback: tuple[float, float]) -> list[dict[str, Any]]:
    ...
    for i in range(count):
        try:
            ...
            if name_el.count() > 0:
                name = name_el.inner_text(timeout=800).strip()
            if not name:
            ...
            if rating_el.count() > 0:
                rating = parse_rating(rating_el.inner_text(timeout=400))
            if rating is None:
            ...
            if review_el.count() > 0:
                reviews = parse_review_count(review_el.inner_text(timeout=400))
            if reviews is None:
...
def extract_place_details(page: Page, listing: dict[str, Any]) -> dict[str, Any]:
    ...
    for selector in ('button[data-item-id="address"]', 'button[aria-label*="Address"]'):
        try:
            ...
            if el.count() > 0:
                addr = el.inner_text(timeout=800).strip()
                if addr:
                    result["address"] = addr
    ...
    for selector in ('button[data-item-id^="phone"]', 'button[aria-label*="Phone"]', 'a[href^="tel:"]'):
        try:
            el = page.locator(selector).first
            if el.count() > 0:
                raw = el.inner_text(timeout=800) or el.get_attribute("href") or ""
...
def scrape_target(page: Page, target: dict[str, str | float]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        feed_found = False
        for sel in ('div[role="feed"]', 'div[aria-label*="Results"]', 'div.Nv2PK', 'div[role="main"]'):
            try:
    ...
    # Deep scroll through all results until the end
    scroll_feed(page, max_scrolls=30)
    listings = parse_list_cards(page, name, (lat, lon))
    print(f"  ↳ Found {len(listings)} total qualifying places (4.0+ ★ & 1K+ reviews)")

    detailed: list[dict[str, Any]] = []
    # Scrapes EVERY qualifying place found (no [:6] limit)
...
def run_scraper(
    ...
    return all_places        return int(val)
    except ValueError:
        return None
...
def scroll_feed(page: Page, times: int = 4) -> None:
...
def scrape_target(page: Page, target: dict[str, str | float]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        # Multi-selector wait to prevent timeout
        feed_found = False
        for sel in ('div[role="feed"]', 'div[aria-label*="Results"]', 'div.Nv2PK', 'div[role="main"]'):
            try:
                page.wait_for_selector(sel, timeout=5000)
    ...
    scroll_feed(page, times=4)
    listings = parse_list_cards(page, name, (lat, lon))
    print(f"  ↳ Found {len(listings)} qualifying places (4.0+ ★ & 1K+ reviews)")

    detailed: list[dict[str, Any]] = []
    for idx, listing in enumerate(listings[:6], start=1):
        print(f"    [{idx}/{min(len(listings), 6)}] Scraping details: {listing['name']} ({listing['rating']}★, {listing['review_count']:,} reviews)...")
        try:
            detailed.append(extract_place_details(page, listing))
            random_delay(0.5, 1.0)
        except Exception as exc:
